    ...
    <main class="relative flex flex-col w-full max-w-[1280px] px-[75px] mx-auto mt-[50px] mb-[62px]">
      ...
      <div class="flex gap-[26px] mt-[30px]">
        ...
        >
          ...
          <div id="Airlanes" class="flex flex-col gap-4">
            <p class="font-semibold">Airlanes</p>
            @foreach ($airlanes as $airlane)
              <label class="flex items-center gap-[10px]">
                <input
                  type="checkbox"
                  name="airlanes"
                  ...
                  class="flex w-6 h-6 shrink-0 appearance-none outline-none rounded-lg ring-1 ring-garuda-black border border-white checked:bg-black checked:border-[5px]"
                />
                <img src="{{ asset('storage/' . $airlane->logo) }}" alt="logo" width="50"/>
                <div class="flex flex-col gap-[2px]">
                  <span class="font-semibold">{{ $airlane->name }}</span>
        ...
        <div id="Result" class="flex flex-col w-full h-fit rounded-3xl p-5 gap-5 bg-white">
          ...
          @foreach ($flights as $flight)
            @if ($flight->segments->count() > 2)
              <div class="transit-card accordion flex flex-col w-full rounded-[20px] border border-garuda-blue py-5 px-6 gap-5 overflow-hidden has-[:checked]:!h-[110px] has-[:checked]:border-[#E8EFF7] hover:!border-garuda-blue transition-all duration-300" >
                <label class="accordion-trigger flex items-center justify-between">
                  ...
                  <div class="flex items-center gap-[10px]">
                    <img
                      src="{{ asset('storage/' . $flight->airlane->logo) }}"
                      class="w-[60px] h-[60px] flex shrink-0"
                      alt="logo"
                    />
                    <div>
                      <p class="font-semibold">{{ $flight->airlane->name }}</p>
                  ...
                  <div class="flex flex-col gap-[2px] items-center justify-center">
                    ...
                    <p class="text-sm text-garuda-grey">Transit {{ $flight->segments->count() - 2}}</p>
                  </div>
                        ...
                        <div class="{{ $loop->first ? 'departure' : ($loop->last ? 'arrival' : 'transit') }} flex items-center gap-5">
                          ...
                          <div class="flex items-center gap-4">
                            <img src="assets/images/icons/{{ $loop->first? 'departure' : ($loop->last ? 'arrival' : 'transit-round-black') }}.svg" 
                                class="w-[50px] h-[50px] flex shirk-0" alt="icon">
                            <div>
                              <p class="text-sm text-garuda-grey mt-[2px]">
              ...
              @else
              <div class="direct-card accordion flex flex-col w-full rounded-[20px] border border-garuda-blue py-5 px-6 gap-5 overflow-hidden has-[:checked]:!h-[110px] has-[:checked]:border-[#E8EFF7] hover:!border-garuda-blue transition-all duration-300">
                <label class="accordion-trigger flex items-center justify-between">
                  <input
                    type="checkbox"
                    name="accordion-input"
                ...
                <div class="accordion-content flex justify-between">
                  <div class="left-content flex flex-col gap-[10px]">
                    <div class="departure flex items-center gap-5">
                      ...
                      <div class="flex items-center gap-4">
                        <img
                          src="assets/images/icons/departure.svg"
                    ...
                    <div class="time flex flex-col items-center w-[83px]">
                      <div
                        class="h-8 border border-garuda-black border-dashed"
                      ></div>
                      <p class="text-xs leading-[18px] text-garuda-grey">
                        {{ number_format($flight->segments->first()->time->diffInHours($flight->segments->last()->time), 0) }} hours
                      </p>
                    ...
                    <div class="arrival flex items-center gap-5">
                      ...
                      <div class="flex items-center gap-4">
                        <img
                          src="assets/images/icons/arrival.svg"
                          class="w-[50px] h-[50px] flex shrink-0"
                          alt="icon"
                        />
                        <div>
